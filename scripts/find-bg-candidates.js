...
function apcaLcRgb(textRgb, bgRgb){
  // Convert sRGB 0..1 to whatever sRGBtoY expects (apca-w3 accepts 0..255 values)
  try {
...
async function run() {
  ...
  const wcagTarget = 4.5; // typical
  const apcaTarget = 60; // user default

  const wcagPassing = [];

  // Sweep L from 0.02 to 0.98 keeping hue and sat similar
  for (let l=0.02; l<=0.98; l+=0.01) {
    const cand = hslToRgb(bgHsl.h, bgHsl.s, l);
    const ratio = wcagContrastRgb(fgRgb, cand);
    const lc = apcaLcRgb(fgRgb, cand);
    const wcagPass = ratio >= wcagTarget;
    if (wcagPass) {
      wcagPassing.push({hex: rgbToHex(cand.r,cand.g,cand.b), ratio: ratio, lc: lc, l});
    }
  }

  // Also try small saturation and hue nudges around bg
  for (let dh=-12; dh<=12; dh+=2) {
    for (let ds=-0.15; ds<=0.15; ds+=0.03) {
      for (let l=0.02; l<=0.98; l+=0.02) {
        ...
        const wcagPass = ratio >= wcagTarget;
        if (wcagPass) {
          wcagPassing.push({hex: rgbToHex(cand.r,cand.g,cand.b), ratio: ratio, lc: lc, h, s, l});
        }
      }
    }
  }

  // Dedupe and sort by APCA absolute value (descending) then closest lightness
  const unique = {};
  for (const r of wcagPassing) unique[r.hex.toLowerCase()] = r;
  const arr = Object.values(unique);
  arr.sort((a,b)=> Math.abs(b.lc || 0) - Math.abs(a.lc || 0) || Math.abs(a.l - bgHsl.l) - Math.abs(b.l - bgHsl.l));

  if (!arr.length) {
    console.log('No WCAG-passing candidates found.');
    process.exit(0);
  }

  console.log('Top WCAG-passing candidates (sorted by APCA Lc magnitude):');
  for (const r of arr.slice(0,24)) {
    console.log(`${r.hex}  WCAG ${r.ratio.toFixed(2)}  APCA ${Number.isNaN(r.lc)?'NaN':r.lc.toFixed(1)}  l:${(r.l*100).toFixed(0)}%`);
  }
}
